...
// `npm run build`, `.next/standalone/apps/web/server.js` can be started as-is
// with all its static assets in place.
const fs = require("node:fs");
const path = require("node:path");

const webDir = path.join(__dirname, "..");
// Next traces the workspace root when tracing a monorepo package, so the
// standalone server lands at `.next/standalone/apps/web/...` rather than
// directly under `.next/standalone/`.
const standaloneAppDir = path.join(webDir, ".next", "standalone", "apps", "web");

if (!fs.existsSync(standaloneAppDir)) {
  console.warn(`[copy-standalone-assets] ${standaloneAppDir} not found — is "output: standalone" set in next.config.ts?`);
  process.exit(0);
}
...
copy(path.join(webDir, "public"), path.join(standaloneAppDir, "public"));
copy(path.join(webDir, ".next", "static"), path.join(standaloneAppDir, ".next", "static"));
